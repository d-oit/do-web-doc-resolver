...
async def resolve_with_mistral_browser_async(
    url: str, max_chars: int = MAX_CHARS
) -> ResolvedResult | None:
    """Async version of Mistral browser resolver."""
    ...
    try:
        ...
        content = await loop.run_in_executor(get_shared_pool(), _sync_extract)
        if not content:
...
async def resolve_with_mistral_websearch_async(
    query: str, max_chars: int = MAX_CHARS
) -> ResolvedResult | None:
    """Async version of Mistral websearch resolver."""
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(get_shared_pool(), _sync_search)
        content = ""
