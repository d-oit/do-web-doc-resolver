  ...
  return (
    <main className="min-h-screen bg-background text-foreground font-mono flex flex-col lg:flex-row">
      ...
      <aside
        ...
        `}
      >
        <div className="flex items-center justify-between border-b-2 border-border-muted h-[44px]">
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="flex-1 px-4 flex items-center justify-between hover:bg-[#141414] transition-colors h-full focus:outline-none"
          ...
          {sidebarOpen && (
            <Link href="/settings" className="px-4 text-[10px] text-text-muted hover:text-accent border-l-2 border-border-muted h-full flex items-center">
              Keys
            </Link>
          )}
        </div>
          ...
          <div className="p-4 flex flex-col gap-8 overflow-y-auto max-h-[calc(100vh-44px)]">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="text-[10px] text-text-dim">
                <span>{isCustomSelection
                  ? `Using custom selection (${selectedProviders.length} selected)`
                  : `Using ${profile} profile (${activeProviders.length} active)`}</span>
                <p className="mt-1">Profile-recommended providers are outlined in green.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                aria-label="Toggle API keys visibility"
              ...
              {apiKeysOpen && (
                <div className="flex flex-col gap-3 pl-2">
                  <div className="flex flex-col gap-2">
                    <label className="flex items-center gap-3 text-[11px] text-text-muted min-h-[44px] py-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={skipCache}
      ...
      {/* Center - Input/Output */}
      <div id="main-content" data-testid="app-loaded" className="flex-1 flex flex-col min-h-0">
        {/* Header */}
